...
# Childers D., N. Grimm, D. Hope, J. Kaye, C. Martin, N. McIntyre, J. Stutz. 2018. Ecological Survey of Central Arizona: a survey of key ecological indicators in the greater Phoenix metropolitan area and surrounding Sonoran desert, ongoing since 1999. Environmental Data Initiative. http://dx.doi.org/10.6073/pasta/0669ee6a71b24abb1ae3827f4ee77f6d. Dataset accessed 2/21/2018.
...
vetted_data <- formatted_data %>% 
  mutate(
    CRAs = ifelse(grepl("143", PackageID), NA, CRAs),
    CRAs = ifelse(grepl("223", PackageID), "LULCC", CRAs),
    CRAs = ifelse(grepl("245", PackageID), NA, CRAs),
    CRAs = ifelse(grepl("247", PackageID), NA, CRAs),
    CRAs = ifelse(grepl("269", PackageID), "DP", CRAs),
    CRAs = ifelse(grepl("270", PackageID), "OMD", CRAs),
    CRAs = ifelse(grepl("271", PackageID), "OMD", CRAs),
    CRAs = ifelse(grepl("273", PackageID), "PC", CRAs),
    CRAs = ifelse(grepl("274", PackageID), "SESD", CRAs),
    CRAs = ifelse(grepl("278", PackageID), sub("LULCC, ", "", CRAs), CRAs),
    ...
    CRAs = ifelse(grepl("617", PackageID), sub("DP, ", "", CRAs), CRAs),
    ...
    CRAs = ifelse(grepl("Lerman", citation), "SESD", CRAs),
    ...
    CRAs = ifelse(grepl("652", PackageID), paste0(CRAs, ", PC"), CRAs),
    CRAs = ifelse(grepl("653", PackageID), paste0(CRAs, ", PC"), CRAs)
  )
```
...
# write_csv(capData, '~/Desktop/CAP_data_inventory.csv', na = "")
# 
# Table X. Inventory of CAP LTER generated datasets housed in the LTER Network data repository hosted by the Environmental Data Initiative. The most relevant LTER Core Research Area(s) are noted (PP = Primary Production; PS = Population Studies; MoOM = Movement of Organic Matter; MoIM = Movement of Inorganic Matter; DP = Disturbance Patterns; LULC = Land Use and Land Cover Change; HEI = Human-Environment Interactions).
...
set.alignment('left')
pandoc.table(vetted_data, split.table = Inf, missing = "")
